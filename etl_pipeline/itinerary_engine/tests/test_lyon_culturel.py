...
def test_lyon_culturel():
    ...
    assert len(culturel_pois) > 0, "Aucun POI culturel trouvé dans lyon_pois.json"
    ...
    # Export JSON
    builder.export_itinerary_to_json(days_itinerary, "lyon_culturel_test.json")

    builder.close()

    print("\nItinéraire Lyon Culturel généré avec succès.")
