...
DATA_FILE = os.path.join("data", "limoux_pois.json")
...
def test_limoux_culturel():
    # Charger les POI de Limoux
    pois = load_pois()

    # Filtrer uniquement les POI Culturel
    culturel_pois = [p for p in pois if "Culturel" in p["themes"]]

    assert len(culturel_pois) > 0, "Aucun POI culturel trouvé dans limoux_pois.json"
    ...
    # Export JSON
    builder.export_itinerary_to_json(days_itinerary, "limoux_culturel_test.json")

    builder.close()

    print("\nItinéraire Limoux Culturel généré avec succès.")
